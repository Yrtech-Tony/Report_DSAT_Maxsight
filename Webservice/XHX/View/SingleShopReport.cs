        ...
        private void simpleButton3_Click(object sender, EventArgs e)
        {
            ...
            foreach (ShopDto shop in _shopDtoList)
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        if (!Directory.Exists(tbnFilePath.Text + @"\" + shop.ShopCode + "_" + shop.ShopName + @"\" + ds.Tables[0].Rows[i]["CharterName"].ToString()))
                        {
                            Directory.CreateDirectory(tbnFilePath.Text + @"\" + shop.ShopCode + "_" + shop.ShopName + @"\" + ds.Tables[0].Rows[i]["CharterName"].ToString());
                        }
                        ...
                        if (picName.Length == 1)
                        {
                            ...
                            if (image != null)
                            {
                                ...
                                //picimage.Save(tbnFilePath.Text + @"\" + shop.ShopCode + "_" + shop.ShopName + @"\" + ds.Tables[0].Rows[i]["CharterName"].ToString() + @"\" + lossDesc.Replace("/", "").Replace("\t\n", "").Replace("\n", "").Replace("\t", "").Replace(":", "")
                                //    .Replace(">","大于").Replace("<","小于")+ ".jpg");
                                picimage.Save(tbnFilePath.Text + @"\" + shop.ShopCode + "_" + shop.ShopName + @"\" + ds.Tables[0].Rows[i]["CharterName"].ToString() + @"\" + ds.Tables[0].Rows[i]["SubjectCode"].ToString() + ".jpg");
                            }
                        }
                        else
                        {
                            for(int j = 0;j<picName.Length;j++)
                            {
                                ...
                                if (image != null)
                                {
                                    ...
                                    MemoryStream buf = new MemoryStream(image);
                                    Image picimage = Image.FromStream(buf, true);
                                    //picimage.Save(tbnFilePath.Text + @"\" + shop.ShopCode + "_" + shop.ShopName + @"\" + ds.Tables[0].Rows[i]["CharterName"].ToString() + @"\"
                                    //    + lossDesc.Replace("/", "").Replace("\t\n", "").Replace("\n", "").Replace("\t", "").Replace(":","")
                                    //    .Replace(">", "大于").Replace("<", "小于") + "_" + (j + 1).ToString() + ".jpg");
                                    picimage.Save(tbnFilePath.Text + @"\" + shop.ShopCode + "_" + shop.ShopName + @"\" + ds.Tables[0].Rows[i]["CharterName"].ToString() + @"\" + ds.Tables[0].Rows[i]["SubjectCode"].ToString() + "_" + (j + 1).ToString() + ".jpg");
                                }
                            }
                        }
            ...
            //    byte[] image1 = service.SearchPicStream(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + shop.ShopName, "1.1.1", "塔式看板一侧远景照片 ");
            //    if (image1 != null)
            //    {
            //        MemoryStream buf1 = new MemoryStream(image1);
            //        Image picimage1 = Image.FromStream(buf1, true);
            //        if (!Directory.Exists(tbnFilePath.Text.Trim() + @"\" + "塔式看板一侧远景照片" + @"\"))
            //        {
            //            Directory.CreateDirectory(tbnFilePath.Text.Trim() + @"\" + "塔式看板一侧远景照片" + @"\");
            //        }
            //        picimage1.Save(tbnFilePath.Text.Trim() + @"\" + "塔式看板一侧远景照片" + @"\" + shop.ShopName + "(" + shop.ShopCode + ")" + "-" + "塔式看板一侧远景照片 " + ".jpg");
            //    }
            //    else
            //    {
            //        log(shop.ShopCode + "-" + shop.ShopName + "塔式看板一侧远景照片");
            //    }

            //    byte[] image2 = service.SearchPicStream(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + shop.ShopName, "1.2.1-4", "收银处全景照片");
            //    if (image2 != null)
            //    {
            //        MemoryStream buf2 = new MemoryStream(image2);
            //        Image picimage2 = Image.FromStream(buf2, true);
            //        if (!Directory.Exists(tbnFilePath.Text.Trim() + @"\" + "收银处全景照片" + @"\"))
            //        {
            //            Directory.CreateDirectory(tbnFilePath.Text.Trim() + @"\" + "收银处全景照片" + @"\");
            //        }
            //        picimage2.Save(tbnFilePath.Text.Trim() + @"\" + "收银处全景照片" + @"\" + shop.ShopName + "(" + shop.ShopCode + ")" + "-" + "收银处全景照片" + ".jpg");

            //    }
            //    else
            //    {
            //        log(shop.ShopCode + "-" + shop.ShopName + "收银处全景照片");
            //    }
            //byte[] image3 = service.SearchPicStream(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + shop.ShopName, "1.2.4-1", "客休区全景");
            //if (image3 != null)
            //{
            //    MemoryStream buf3 = new MemoryStream(image3);
            //    Image picimage3 = Image.FromStream(buf3, true);
            //    if (!Directory.Exists(tbnFilePath.Text.Trim() + @"\" + "客休区全景" + @"\"))
            //    {
            //        Directory.CreateDirectory(tbnFilePath.Text.Trim() + @"\" + "客休区全景" + @"\");
            //    }
            //    picimage3.Save(tbnFilePath.Text.Trim() + @"\" + "客休区全景" + @"\" + shop.ShopName + "(" + shop.ShopCode + ")" + "-" + "客休区全景" + ".jpg");

            //}
            //else
            //{
            //    log(shop.ShopCode + "-" + shop.ShopName + "客休区全景");
            //}
            //byte[] image4 = service.SearchPicStream(CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + shop.ShopName, "1.3.1", "车间地面墙面全景照");
            //if (image4 != null)
            //{
            //    MemoryStream buf4 = new MemoryStream(image4);
            //    Image picimage4 = Image.FromStream(buf4, true);
            //    if (!Directory.Exists(tbnFilePath.Text.Trim() + @"\" + "车间地面墙面全景照" + @"\"))
            //    {
            //        Directory.CreateDirectory(tbnFilePath.Text.Trim() + @"\" + "车间地面墙面全景照" + @"\");
            //    }
            //    picimage4.Save(tbnFilePath.Text.Trim() + @"\" + "车间地面墙面全景照" + @"\" + shop.ShopName + "(" + shop.ShopCode + ")" + "-" + "车间地面墙面全景照" + ".jpg");
            //}
            //else
            //{
            //    log(shop.ShopCode + "-" + shop.ShopName + "车间地面墙面全景照");
            //}
            //}
            #endregion
