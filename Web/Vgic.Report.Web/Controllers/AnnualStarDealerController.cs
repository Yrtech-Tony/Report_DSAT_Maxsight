...
{
    public class AnnualStarDealerController : BaseController
    {
        // GET: CSSOEM
        ...
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/AnnualStarDealer/QuarterReport");//季度报告所在的文件路径
        ...
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/AnnualStarDealer/Complaint");//月度报告所在的文件路径
        ...
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/AnnualStarDealer/DealerGuide");//指导手册所在的文件路径
            ...
            try
            {
                ...
                foreach (ShopDto strShop in list)
                {
                    foreach (FileInfo file in filesInfo)
                    {
                        ...
                        else if (quarterormonth == "Q0" || quarterormonth == "0")
                        {
                            if (file.Name.Split('_')[1] == year
                               && file.Name.Split('_')[0].Trim() == strShop.ShopCode.Trim()
                               && (file.Name.Split('_')[2].Trim() == "Annual" || file.Name.Split('_')[2].Trim() == "YTD"))
                            {
                                ShopDto result = new ShopDto();
                                result.ShopCode = strShop.ShopCode;
                                result.ShopName = strShop.ShopName;
        ...
        public ActionResult DownloadFiles(string areaCode, string groupCode, string shopCode, string year, string quarterormonth, string type)
        {
            try
            {
                string reportPath = HttpContext.Server.MapPath("~/ReportFiles/AnnualStarDealer/");
