...
{
    public class CSSOEMController : BaseController
    {
        // GET: CSSOEM
        ...
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/CSSOEM/QuarterReport");//季度报告所在的文件路径
        ...
        public ActionResult MonthSearch(string areaCode, string groupCode, string shopCode, string year, string quarterormonth, int pageNum)
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/CSSOEM/MonthReport");//月度报告所在的文件路径
        ...
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/CSSOEM/Complaint");//月度报告所在的文件路径
            List<ShopDto> resultListTemp = GetFileList(areaCode, groupCode, shopCode, year, quarterormonth, reportPath);//获取符合条件的所有月度报告
            List<ShopDto> resultList = ((from u in resultListTemp orderby u.ShopCode select u).Skip(_countPerPage * (pageNum - 1)).Take(_countPerPage)).ToList<ShopDto>();//分页
            return Json(new { shopInfoList = resultList, totalCount = resultListTemp.Count }, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            string reportPath = HttpContext.Server.MapPath("~/ReportFiles/CSSOEM/DealerGuide");//指导手册所在的文件路径
        ...
        public ActionResult DownloadFiles(string areaCode, string groupCode, string shopCode, string year, string quarterormonth,string type)
        {
            try
            {
                string reportPath = HttpContext.Server.MapPath("~/ReportFiles/CSSOEM/");
